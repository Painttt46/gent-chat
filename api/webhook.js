...
export default async function handler(req, res) {
  ...
  try {
    ...
    const systemInstruction = {
      parts: [{
        text: `You are Gent, a friendly and concise AI work assistant in Microsoft Teams. 
- You can access employee calendars using their name.
- Your responses MUST start with "FORMAT:TEXT" for simple chat, or "FORMAT:CARD" for structured data like lists, steps, or detailed information.`
      }]
    };
    ...
    // --- START: บล็อกโค้ดที่ปรับปรุงใหม่ ---
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      try {
        if (call.name === "get_user_calendar") {
          ...
          if (!nameOrEmail) {
            functionResponsePayload = { error: "User name or email not provided by the model." };
          } else {
            const calendarData = await getUserCalendar(nameOrEmail);
            console.log("Calendar data received:", calendarData);

            // ✨ ปรับปรุงตรงนี้: จัดโครงสร้างข้อมูลใหม่ให้ AI เข้าใจง่าย
            if (calendarData.error) {
              // ถ้ามี error จากฟังก์ชัน getUserCalendar
              functionResponsePayload = { error: calendarData.error };
            } else {
              // ถ้าสำเร็จ ส่งกลับไปเฉพาะ Array ของ events
              functionResponsePayload = { events: calendarData.value || [] };
            }
          }
        } else {
          functionResponsePayload = { error: "Unknown function called." };
        }

        console.log("Sending clean function response to AI:", JSON.stringify(functionResponsePayload, null, 2));
